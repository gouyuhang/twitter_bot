...
async function sayHi(event, oauth) {
  ...
  var options = {
    url: "http://120.25.81.83:16999/ctp/chat/api",
  ...
  var result=request.post.sync(options,function(error, response, body) {
     })
  s=`Hello ${result['body']['data']['reply']}`
    ...
    json: {
      event: {
        ...
        message_create: {
          ...
          message_data: {
              text:`${result['body']['data']['reply']}`
            //text: `Hi @${senderScreenName}! 👋 your message: ${message.message_create.message_data.text} have been received`,
          },
        },
      },
    },
